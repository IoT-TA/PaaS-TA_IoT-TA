    ...
    create(context) {
        ...
        return {
            ...
            'NewExpression:exit': (node) => {
                if (utils_1.isRegExpConstructor(node)) {
                    regexNodes.push(node);
                }
            },
            'CallExpression:exit': (node) => {
                const callExpr = node;
                if (utils_1.isRequiredParserServices(services) && utils_1.isStringRegexMethodCall(callExpr, services)) {
                    regexNodes.push(callExpr.arguments[0]);
                }
                else if (utils_1.isRegExpConstructor(callExpr)) {
...
function checkRegexComplexity(regexNode, threshold, context) {
    for (const regexParts of findRegexParts(regexNode, context)) {
        ...
        if (complexity > threshold) {
            context.report({
                message: utils_1.toEncodedMessage(`Simplify this regular expression to reduce its complexity from ${complexity} to the ${threshold} allowed.`, secondaryLocations, secondaryMessages, complexity - threshold),
                node: regexParts[0],
            });
        }
    }
...
class RegexPartFinder {
    ...
    find(node) {
        ...
        else if (utils_1.isRegexLiteral(node)) {
            this.parts.push([node]);
        }
        else if (utils_1.isStringLiteral(node)) {
            this.parts.push([node]);
        }
        else if (utils_1.isStaticTemplateLiteral(node)) {
            this.parts.push([node]);
        }
        else if (utils_1.isIdentifier(node)) {
            const initializer = utils_1.getUniqueWriteUsage(this.context, node.name);
            if (initializer) {
                this.find(initializer);
            }
        }
        else if (utils_1.isBinaryPlus(node)) {
            const literals = [];
...
class ComplexityCalculator {
    constructor(regexPart, context) {
        ...
        this.complexity = 0;
        this.components = [];
        this.regexPartAST = utils_1.getParsedRegex(regexPart, context);
    }
    visit() {
        ...
        regexpp.visitRegExpAST(this.regexPartAST, {
            onAssertionEnter: (node) => {
                ...
                if (node.kind === 'lookahead' || node.kind === 'lookbehind') {
                    const [start, end] = utils_1.getRegexpRange(this.regexPart, node);
            ...
            onQuantifierEnter: (node) => {
                /* repetition */
                const [start] = utils_1.getRegexpRange(this.regexPart, node);
                const [, end] = utils_1.getRegexpRange(this.regexPart, node.element);
    ...
    increaseComplexity(increment, node, offset) {
        ...
        const loc = utils_1.getRegexpLocation(this.regexPart, node, this.context, offset);
    ...
    onDisjunctionEnter(node) {
        if (node.alternatives.length > 1) {
            ...
            while (alternatives.length > 1) {
                const [start, end] = utils_1.getRegexpRange(this.regexPart, alternatives[1]);
