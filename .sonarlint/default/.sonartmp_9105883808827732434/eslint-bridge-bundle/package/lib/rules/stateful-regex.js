...
function extractRegex(node, acc) {
    if (utils_1.isRegexLiteral(node)) {
        const { flags } = node.regex;
        acc.push({ node, flags });
    }
    else if (utils_1.isRegExpConstructor(node)) {
        const flags = utils_1.getFlags(node) || '';
        acc.push({ node, flags });
    }
}
function extractRegexInvocation(callExpr, regexes, invocations, context) {
    if (utils_1.isCallingMethod(callExpr, 1, 'exec', 'test') &&
        callExpr.callee.object.type === 'Identifier') {
        const { object } = callExpr.callee;
        const variable = utils_1.getVariableFromName(context, object.name);
        if (variable) {
            const value = utils_1.getUniqueWriteUsage(context, variable.name);
...
function extractResetRegex(node, regexes, resets, context) {
    ...
    if (utils_1.isDotNotation(node) &&
        ...
        if ((parent === null || parent === void 0 ? void 0 : parent.type) === 'AssignmentExpression' && parent.left === node) {
            const variable = utils_1.getVariableFromName(context, node.object.name);
            if (variable) {
                const value = utils_1.getUniqueWriteUsage(context, variable.name);
                const regex = regexes.find(r => r.node === value);
                if (regex) {
...
function checkWhileConditionRegex(callExpr, context) {
    ...
    if (utils_1.isMethodCall(callExpr)) {
        const { object, property } = callExpr.callee;
        if ((utils_1.isRegexLiteral(object) || utils_1.isRegExpConstructor(object)) && property.name === 'exec') {
            const flags = object.type === 'Literal' ? object.regex.flags : utils_1.getFlags(object);
            if (flags && flags.includes('g') && isWithinWhileCondition(callExpr, context)) {
                context.report({
                    message: utils_1.toEncodedMessage('Extract this regular expression to avoid infinite loop.', []),
                    node: object,
                });
            }
        }
...
function checkGlobalStickyRegex(regex, context) {
    ...
    if (regex.flags.includes('g') && regex.flags.includes('y')) {
        context.report({
            message: utils_1.toEncodedMessage(`Remove the 'g' flag from this regex as it is shadowed by the 'y' flag.`, []),
            node: regex.node,
        });
    }
}
function checkMultipleInputsRegex(regex, usages, resets, context) {
    ...
    if (!resets.has(regex)) {
        ...
        if (definition && uniqueInputs.size > 1 && !regexReset) {
            ...
            context.report({
                message: utils_1.toEncodedMessage(`Remove the 'g' flag from this regex as it is used on different inputs.`, usages, usages.map((_, idx) => `Usage ${idx + 1}`)),
                node: pattern,
            });
        }
    }
