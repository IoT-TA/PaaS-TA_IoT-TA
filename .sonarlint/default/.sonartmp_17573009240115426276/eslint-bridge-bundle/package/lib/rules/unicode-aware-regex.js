            ...
            if (!isUnicodeEnabled && (unicodeProperties.length > 0 || unicodeCharacters.length > 0)) {
                ...
                unicodeProperties.forEach(p => {
                    secondaryLocations.push({
                        loc: utils_1.getRegexpLocation(context.node, p.character, context, [0, p.offset]),
                    });
                    secondaryMessages.push('Unicode property');
                });
                unicodeCharacters.forEach(c => {
                    secondaryLocations.push({ loc: utils_1.getRegexpLocation(context.node, c, context) });
                    secondaryMessages.push('Unicode character');
                });
                context.reportRegExpNode({
                    message: utils_1.toEncodedMessage(`Enable the 'u' flag for this regex using Unicode constructs.`, secondaryLocations, secondaryMessages),
                    node: context.node,
                    regexpNode: regexp,
                });
            }
